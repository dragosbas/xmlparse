...
def upload_file():
    if request.method == 'POST':
        raport = '--Raportul a fost generat in data de ' + time.strftime("%d/%m/%Y") + ' la ora ' + time.strftime(
            "%H:%M:%S") + '\n'
        ...
        try:
            shutil.unpack_archive(os.path.join(upload_folder, uploaded_file.filename), os.path.join(upload_folder),
                                  "tar")
            ...
            try:
                for filename in os.listdir(upload_folder):
                    with open(os.path.join(upload_folder, filename), 'rt', encoding="utf8") as currentfile:
                        ...
                        CUI = file_as_xml.get('XmlReport', {}).get('Header', {}).get('Angajator', {}).get('Detalii',
                                                                                                          {}).get('Cui',
                                                                                                                  '0001')
                        if CUI in ['0001', '']:
                            return Flask.response_class(
                                "Structura fisierului nu este recunoscuta, verificati fisierul sursa", status=401,
                                mimetype='application/json')
                        if CUI != APPROVED_CUI:
                            return Flask.response_class(
                                "CUI-ul din fisier nu corespunde companiei pentru care s-a propus incarcarea",
                ...
                return Flask.response_class("Eroare la concatenarea fisierelor din rvs", status=401,
                                            mimetype='application/json')
        else:
            try:
                with open(os.path.join(upload_folder, uploaded_file.filename), 'rt', encoding="utf8") as currentfile:
                    ...
                    if CUI != APPROVED_CUI:
                        return Flask.response_class(
                            "CUI-ul din fisier nu corespunde companiei pentru care s-a propus incarcarea", status=401,
                            mimetype='application/json')
                    ...
                    if AN_DIN_FISIER != PERIOADA[:4] or int(LUNA_DIN_FISIER) != date_formater.get(PERIOADA[5:7], ""):
                        return Flask.response_class(
                            f"Perioada nu se potriveste. Perioada din fisier este {PERIOADA_DIN_FISIER}, perioada ceruta este {PERIOADA[-7:]}",
                            status=401, mimetype='application/json')
        ...
        if result['is_rvs']:
            # try:
            processed_database = process2(xmlData, lista_cnp_crypt=lista_cnp_crypt, lista_cor_exclus=lista_cor_exclus,
                                          perioada=PERIOADA, cui=CUI, minCor=MINCOR)
        # except:
        #     return Flask.response_class("Problems processing xml files from rvs", status=401, mimetype='application/json')
        if result['is_d112']:
            # try:
            processed_database = process1(xmlData, lista_cnp_crypt=lista_cnp_crypt, lista_cor_exclus=lista_cor_exclus,
                                          perioada=PERIOADA, cui=CUI, minCor=MINCOR)
        # except:
...
def process1(xmlData={}, lista_cnp_crypt=[], lista_cor_exclus=[], perioada='2000-01', cui='0001', minCor=1):
    ...
    SINGLEKEYS = ['idAsig', 'CUI', 'Perioada', 'cnpAsig', 'dataAng', 'dataSf', 'A_1', 'A_2', 'A_3', 'A_4', 'A_5', 'A_6',
                  'A_7', 'A_8', 'C_1']
    MINKEYS = ['B1_1', 'B1_3', 'E3_1', 'E3_2', 'E3_4']
    SUMKEYS = ['B1_2', 'B1_4', 'B1_5', 'B1_6', 'B1_7', 'B1_8', 'B2_2', 'B2_3', 'B2_4', 'B2_5', 'B2_6', 'B2_7', 'B2_6i',
               'B2_6f', 'B2_7i', 'B2_7f', 'B3_1', 'B3_2', 'B3_3', 'B3_4', 'B3_5', 'B3_6', 'B3_7', 'B3_8', 'B3_9',
               'B3_10', 'B3_11', 'B3_12', 'B3_13', 'D_14', 'D_15', 'D_20', 'D_21', 'E3_8', 'E3_10', 'E3_16']
...
def process2(xmlData, lista_cnp_crypt, lista_cor_exclus, perioada, cui, minCor=1):
    ...
    for salariat in xmlData:
        ...
        for contract in contract_list:
            ...
            contract_export['StareCurentaDataIncetareDetasare'] = contract.get('StareCurenta', {}).get(
                'DataIncetareDetasare', "")
            contract_export['StareCurentaDataIncetareSuspendare'] = contract.get('StareCurenta', {}).get(
    ...
    for id_contract, contract in temp_export_contracte.items():
        if contract.get('IdSalariat') not in id_salariati_export or contract.get("Radiat") == 'true' or contract.get(
                'CorCod', "") in lista_cor_exclus:
            continue
